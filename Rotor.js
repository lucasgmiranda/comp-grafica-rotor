...
/**
 * helicele to a buffer on the GPU.
 * @type {WebGLBuffer}
 */
var vertexBuffer;
...
/**
 * helicele to the compiled shader program on the GPU.
 * @type {WebGLProgram}
 */
var lightingShader;
 ...
 * @type {Matrix4}
 */
var corpoMatrix = new Matrix4()
  .setTranslate(0, 0, 0)
  .rotate(joint.corpo, 0, 1, 0);
...
/**  @type {Matrix4} */
var rotorPinoMatrix = new Matrix4()
  .setTranslate(0, 0, 3)
  .translate(0, 2.5, 1.0)
  .rotate(-joint.rotorPino, 1, 0, 0)
  .translate(0, -2.5, -1.0);

var rotorMatrix = new Matrix4()
  .setTranslate(0, 0, 3)
  .translate(0, 2.5, -2)
  .rotate(-joint.rotor, 1, 0, 0)
  .translate(0, -2.5, -1.0);

/**  @type {Matrix4} */
var heliceMatrix = new Matrix4()
  .setTranslate(0, 0, 0.3)
  .rotate(joint.helice, 0, 1, 0);

/**  @type {Matrix4} */
var caixaMatrix = new Matrix4()
  .setTranslate(0, 12, 0)
  .rotate(joint.caixa, 0, 1, 0);

var corpoMatrixLocal = new Matrix4().setScale(2.5, 30, 2.5);
var baseMatrixLocal = new Matrix4().setScale(7, 2, 7);
var rotorPinoMatrixLocal = new Matrix4().setScale(1.5, 1.5, 2.5);
var rotorMatrixLocal = new Matrix4().setScale(2, 1.8, 1.8);
var heliceMatrixLocal = new Matrix4().setScale(20, 1, 0.5);
var caixaMatrixLocal = new Matrix4().setScale(4.5, 4.5, 4.5);
...
/**
 * <p>heliceler for keydown events.</p>
 * Adjusts object rotations.
 * @param {KeyboardEvent} event keyboard event.
 */
function heliceleKeyPress(event) {
  ...
  switch (ch) {
    case "t":
      joint.corpo += 15;
      corpoMatrix.setTranslate(0, -2, 0).rotate(joint.corpo, 0, 1, 0);
      break;
    case "T":
      joint.corpo -= 15;
      corpoMatrix.setTranslate(0, 2, 0).rotate(joint.corpo, 0, 1, 0);
...
function draw(useRotator = true) {
  ...
  var s = new Stack();
  s.push(corpoMatrix);
  renderCube(s, corpoMatrixLocal);

  s.push(new Matrix4(s.top()).multiply(caixaMatrix));
  renderCube(s, caixaMatrixLocal);

  s.push(new Matrix4(s.top()).multiply(rotorPinoMatrix));
  renderCube(s, rotorPinoMatrixLocal);

  s.push(new Matrix4(s.top()).multiply(rotorMatrix));
  renderCube(s, rotorMatrixLocal);


  s.push(new Matrix4(s.top()).multiply(heliceMatrix));
  renderCube(s, heliceMatrixLocal);
  s.pop();
  s.pop();
  s.pop();
  s.pop();
...
window.addEventListener("load", (event) => {
  ...
  (function animate() {
    ...
    if (is_rolling) {
      rotorPinoMatrix.rotate(rotacao, 0, 0, 1);
    } else {
      rotorPinoMatrix.rotate(0, 0, 0, 1);
    }
